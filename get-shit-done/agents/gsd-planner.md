...
@~/.openclaw/get-shit-done/references/mandatory-initial-read.md
...
**Project instructions:** Read `./OPENCLAW.md` if it exists in the working directory. Follow all project-specific guidelines, security requirements, and coding conventions.

**Project skills:** @~/.openclaw/get-shit-done/references/project-skills-discovery.md
...
@~/.openclaw/get-shit-done/references/planner-source-audit.md for full format, examples, and gap-handling rules.
...
@~/.openclaw/get-shit-done/references/planner-source-audit.md for constraint examples.
...
**Test:** Could a different Claude instance execute without asking clarifying questions? If not, add specificity. See @~/.openclaw/get-shit-done/references/planner-antipatterns.md for vague-vs-specific comparison table.

## TDD Detection

**When `workflow.tdd_mode` is enabled:** Apply TDD heuristics aggressively — all eligible tasks MUST use `type: tdd`. Read @~/.openclaw/get-shit-done/references/tdd.md for gate enforcement rules and the end-of-phase review checkpoint format.
...
<execution_context>
@~/.openclaw/get-shit-done/workflows/execute-plan.md
@~/.openclaw/get-shit-done/templates/summary.md
</execution_context>
...
@~/.openclaw/get-shit-done/references/planner-antipatterns.md
...
```bash
gsd-sdk query graphify query "<phase-goal-keyword>" --budget 2000
```

(graphify is not exposed on `gsd-sdk query` yet; use `gsd-sdk query` for graphify only.)
...
@~/.openclaw/get-shit-done/references/thinking-models-planning.md
...
See @~/.openclaw/get-shit-done/references/planner-chunked.md for `## OUTLINE COMPLETE` and `## PLAN COMPLETE` return formats used in chunked mode.
