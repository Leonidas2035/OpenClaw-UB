...
- ./OPENCLAW.md (if exists — project-specific guidelines)
...
- ./OPENCLAW.md (if exists — follow project-specific guidelines)
...
- .planning/STATE.md (Project state)
- ./OPENCLAW.md (Project instructions, if exists)
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `gsd-sdk query commit` command (or legacy `gsd-sdk query` commit) handles already-committed files gracefully.
