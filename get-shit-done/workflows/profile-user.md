...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `gsd-sdk query` handlers (with legacy `gsd-sdk query` parity where needed) and the gsd-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.openclaw/get-shit-done/references/ui-brand.md (display patterns)
- @$HOME/.openclaw/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @$HOME/.openclaw/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.openclaw/get-shit-done/USER-PROFILE.md"
...
```bash
cp "$HOME/.openclaw/get-shit-done/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.openclaw/get-shit-done/USER-PROFILE.md
...
- The user-profiling reference doc at `$HOME/.openclaw/get-shit-done/references/user-profiling.md`
...
Reference: @$HOME/.openclaw/get-shit-done/references/user-profiling.md
...
Display: "✓ Profile written to $HOME/.openclaw/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global CLAUDE.md" -- "Add profile to $HOME/.claude/OPENCLAW.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.openclaw/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
Display: "✓ Added profile section to $HOME/.claude/OPENCLAW.md"

**Error handling:** If any `gsd-sdk query` or gsd-sdk query call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
Your profile:    $HOME/.openclaw/get-shit-done/USER-PROFILE.md
```
...
Artifacts:
  ...
  ✓ CLAUDE.md section       ./OPENCLAW.md
  ✓ Global CLAUDE.md        $HOME/.claude/OPENCLAW.md
```
...
- [ ] Artifacts generated sequentially via gsd-sdk query (or gsd-sdk query) subcommands
