...
@~/.openclaw/get-shit-done/references/git-integration.md
</required_reading>
...
See `~/.openclaw/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.openclaw/get-shit-done/references/checkpoints.md for details.
</step>
...
If `NODE_REPAIR` is `true`: invoke `@./.openclaw/get-shit-done/workflows/node-repair.md` with:
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.openclaw/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.openclaw/get-shit-done/templates/summary.md`.
