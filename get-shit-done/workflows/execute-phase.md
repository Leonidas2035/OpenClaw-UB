...
@~/.openclaw/get-shit-done/references/context-budget.md
@~/.openclaw/get-shit-done/references/gates.md
</required_reading>

<available_agent_types>
These are the valid GSD subagent types registered in .openclaw/get-shit-done/agents/ (or equivalent for your runtime).
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.openclaw/get-shit-done/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.openclaw/get-shit-done/references/planner-antipatterns.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.openclaw/get-shit-done/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       For `gsd-sdk query commit` (or legacy `gsd-sdk query` commit): add --no-verify flag when needed.
       ...
       @~/.openclaw/get-shit-done/references/checkpoints.md
       @~/.openclaw/get-shit-done/references/tdd.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.openclaw/get-shit-done/references/executor-examples.md'}
       </execution_context>
       ...
       ` : ''}
       - ./OPENCLAW.md (Project instructions, if exists — follow project-specific guidelines and coding conventions)
       - .openclaw/skills/ or .agents/skills/ (Project skills, if either exists — list skills, read SKILL.md for each, follow relevant rules during implementation)
       </files_to_read>
...
Read and follow `~/.openclaw/get-shit-done/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
