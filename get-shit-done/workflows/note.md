...
- **Global scope**: `~/.openclaw/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `~/.openclaw/notes/`)
...
2. Glob `~/.openclaw/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `~/.openclaw/notes/` — works in any directory
