...
global: `~/.openclaw/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, `~/.config/kilo/`, or `~/.codex/`
...
when `gsd-sdk` is on `PATH`, or the bundled `gsd-sdk query detect-custom-files`
...
```bash
GSD_TOOLS="$RUNTIME_DIR/get-shit-done/bin/gsd-sdk query"
CUSTOM_JSON=''
